...
function MainMenuControls(props: Props) {
  if (saveFileExists()) {
    return (
      <>
        <div className="text-padded">{getRandomMantra()}</div>
        <Button
          className="button primary-action-button title-button"
          onClick={props.startSavedGame}
    ...
    return (
      <>
        <div className="text-padded">{getInitialMantra()}</div>
        <Button
          className="button primary-action-button title-button"
